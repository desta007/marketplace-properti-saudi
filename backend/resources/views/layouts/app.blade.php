        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-16">
                ...
                <nav class="hidden md:flex items-center gap-8">
                    <a href="{{ route('home') }}"
                        class="text-gray-600 hover:text-emerald-600 font-medium transition-colors {{ request()->routeIs('home') ? 'text-emerald-600' : '' }}">
                        {{ __('nav.home') }}
                    </a>
                    <a href="{{ route('properties.index') }}"
                        class="text-gray-600 hover:text-emerald-600 font-medium transition-colors {{ request()->routeIs('properties.*') ? 'text-emerald-600' : '' }}">
                        {{ __('nav.properties') }}
                    </a>
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="relative" x-data="{ open: false }">
                        ...
                        <div x-show="open" @click.away="open = false" x-transition
                            class="absolute end-0 mt-2 w-40 bg-white rounded-xl shadow-lg border overflow-hidden z-50">
                            <a href="{{ route('locale.switch', 'en') }}"
                                class="w-full px-4 py-3 text-start hover:bg-gray-50 flex items-center gap-3 {{ app()->getLocale() === 'en' ? 'bg-emerald-50' : '' }}">
                                <span>🇬🇧</span> English
                            </a>
                            <a href="{{ route('locale.switch', 'ar') }}"
                                class="w-full px-4 py-3 text-start hover:bg-gray-50 flex items-center gap-3 {{ app()->getLocale() === 'ar' ? 'bg-emerald-50' : '' }}">
                                <span>🇸🇦</span> العربية
                            </a>
                        </div>
                    </div>
                    ...
                    @guest
                        <a href="{{ route('login') }}"
                            class="hidden sm:inline-flex px-5 py-2.5 bg-emerald-600 text-white font-medium rounded-xl hover:bg-emerald-700 transition-colors">
                            {{ __('nav.login') }}
                        </a>
                    @else
                        <div class="relative" x-data="{ open: false }">
                            ...
                            <div x-show="open" @click.away="open = false" x-transition
                                class="absolute end-0 mt-2 w-48 bg-white rounded-xl shadow-lg border overflow-hidden z-50">
                                <a href="{{ route('profile') }}"
                                    class="block px-4 py-3 text-start hover:bg-gray-50">{{ __('nav.profile') }}</a>
                                <a href="{{ route('favorites') }}"
                                    class="block px-4 py-3 text-start hover:bg-gray-50">{{ __('nav.favorites') }}</a>
                                ...
                                @if(auth()->user()->isAdmin())
                                    <a href="{{ route('admin.dashboard') }}"
                                        class="block px-4 py-3 text-start hover:bg-gray-50 text-blue-600">⚙️ Admin Panel</a>
                                @endif
                                <hr class="my-1">
                                <form action="{{ route('logout') }}" method="POST">
                                    @csrf
                                    <button type="submit"
                                        class="w-full px-4 py-3 text-start hover:bg-gray-50 text-red-600">{{ __('nav.logout') }}</button>
                                </form>
                            </div>
                        </div>
                    @endguest
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
                ...
                <div>
                    ...
                    <ul class="space-y-2 text-gray-400">
                        <li><a href="{{ route('properties.index', ['type' => 'villa']) }}"
                                class="hover:text-emerald-400 transition-colors">{{ __('types.villa') }}</a></li>
                        <li><a href="{{ route('properties.index', ['type' => 'apartment']) }}"
                                class="hover:text-emerald-400 transition-colors">{{ __('types.apartment') }}</a></li>
                        <li><a href="{{ route('properties.index', ['type' => 'compound']) }}"
                                class="hover:text-emerald-400 transition-colors">{{ __('types.compound') }}</a></li>
                        <li><a href="{{ route('properties.index', ['type' => 'land']) }}"
                                class="hover:text-emerald-400 transition-colors">{{ __('types.land') }}</a></li>
                    </ul>
                </div>
                <div>
